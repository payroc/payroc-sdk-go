...
Write-Host "Installing Go extension..." -ForegroundColor Cyan
code --install-extension golang.go --force
if ($LASTEXITCODE -eq 0) {
    Write-Host "Go extension installed" -ForegroundColor Green
} else {
    Write-Host "Failed to install Go extension" -ForegroundColor Red
}
...
Write-Host "2. Install Go tools when prompted by the extension" -ForegroundColor White
